...
earthquakes %>%
  ...
  select(place, mag) %>%
  mutate(percentile = ntile(mag, 100)) %>%
  arrange(place, desc(mag)) %>%
  collect(n = 5) %>%
  kable()
```
...
Rather than preceding the `mutate` with a `window_order()`, we include the variable that we wish to sort by (`mag`) as the first argument to `ntile()`.
