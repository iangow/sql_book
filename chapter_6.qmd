...
```{r}
japan_quakes %>%
  summarize(ntile_25 = quantile(mag, probs = 0.25, na.rm = TRUE),
            median = quantile(mag, probs = 0.50, na.rm = TRUE),
            ntile_75 = quantile(mag, probs = 0.75, na.rm = TRUE)) %>%
  mutate(iqr = (ntile_75 - ntile_25) * 1.5,
         lower_whisker = ntile_25 - (ntile_75 - ntile_25) * 1.5,
         upper_whisker = ntile_75 + (ntile_75 - ntile_25) * 1.5) %>%
  kable()
```
