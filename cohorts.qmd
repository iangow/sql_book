...
#| include: false
duckdb_read_csv <-
  function (conn, name, files, header = TRUE, na.strings = "", 
            nrow.check = 500, delim = ",", quote = "\"", col.names = NULL, 
            lower.case.names = FALSE, sep = delim, transaction = TRUE, use.readr = FALSE,
            ...) {
    ...
    if (!use.readr) {
      ...
      headers <- lapply(files, readr::read_delim, delim = delim, na = na.strings, 
                        quote = quote, n_max = nrow.check,
                        guess_max = nrow.check, col_names = header, 
    ...
    if (length(files) > 1) {
      ...
      if (!all(nms == nms[, 1])) 
        stop("Files have different variable names")
      types <- sapply(headers, function(df) sapply(df, dbDataType, 
                                                   dbObj = conn))
      if (!all(types == types[, 1])) 
        stop("Files have different variable types")
    }
    if (transaction) {
      dbBegin(conn)
    ...
    if (!dbExistsTable(conn, tablename)) {
      ...
      if (!is.null(col.names)) {
        ...
        if (length(unique(col.names)) != length(names(headers[[1]]))) {
          stop("You supplied ", length(unique(col.names)), 
               " unique column names, but file has ", length(names(headers[[1]])), 
               " columns.")
        }
    ...
    for (i in seq_along(files)) {
      ...
      dbExecute(conn, sprintf("COPY %s FROM %s (DELIMITER %s, QUOTE %s, HEADER %s, NULL %s)", 
                              tablename, thefile, dbQuoteString(conn, delim), dbQuoteString(conn, 
                                                                                            quote), tolower(header), dbQuoteString(conn, 
                                                                                                                                   na.strings[1])))
    }
...
knit_print.tbl_sql <- function(x, ...) {
  knitr::knit_print(knitr::kable(collect(x, n = 3)))
}
...
duckdb_read_csv(db, "legislators",
                files = "data/legislators.csv", 
                use.readr = TRUE, nrow.check = 5000)
duckdb_read_csv(db, "legislators_terms", 
                files = "data/legislators_terms.csv",
                use.readr = TRUE)

legislators <- tbl(db, "legislators")
legislators_terms <- tbl(db, "legislators_terms")
```

### Exploring the data

There are two tables in the *legislators* data set.
The `legislators` data looks to have `id_bioguide` as a primary key.
