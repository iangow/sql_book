...
**INSERT CODE TO GET DATA SET HERE.**

```{r}
#| include: false
#| eval: false
retail_sales <- tbl(pg, "retail_sales") %>% collect()

# dbDisconnect(pg)
# pg <- dbConnect(duckdb::duckdb())

retail_sales <- copy_to(pg, retail_sales_local, 
...
We could specify `temporary = FALSE` if we wanted the data to be there permanently.^[Obviously this would not make sense if `pg` is a connection to an in-memory database.]
...
```{r}
#| eval: false
retail_sales_alt <- copy_inline(pg, retail_sales_local)
```
...
date_dim <-
  tibble(date = seq(as.Date('1993-01-01'), 
                    as.Date('2020-12-01'), 
                    by = "1 month")) %>%
  copy_to(pg, ., overwrite = TRUE, name = "date_dim")
```
