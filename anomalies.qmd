  ...
  function (conn, name, files, header = TRUE, na.strings = "", 
    ...
    if (!use.readr) {
      ...
      headers <- lapply(files, readr::read_delim, delim = delim, na = na.strings, 
                        quote = quote, n_max = nrow.check, guess_max = nrow.check, col_names = header, 
...
```{r}
#| warning: false
dbfile <- "data/earthquakes.duckdb"

if (!file.exists(dbfile)) {
  db <- dbConnect(duckdb::duckdb(), dbdir = dbfile, read_only = FALSE)

  files <- list.files("data", "earthquakes.*csv", full.names = TRUE)
  duckdb_read_csv(db, "earthquakes", files, use.readr = TRUE,
                col_types = list(time = col_datetime()))
