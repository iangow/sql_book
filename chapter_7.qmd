...
We need to turn `variant` into the row names of the contingency table so that we only a $ 2 \times 2$ table as out data frame and we use `column_to_rownames()` to this effect.
We then pipe the result into the base R function `chisq.test()`.
I specified `correct = FALSE` so that my result matched what I got from the [online calculator](https://www.socscistatistics.com/tests/chisquare2/) I found.
I think display the Chi-squared statistic and the $p$-value.
