...
However, in compiling a version of this chapter, I could do it in about 24 seconds *without* these lines (i.e., using local data frames) and in about 4 seconds *with* these lines (i.e., using DuckDB).^[Applying `as.data.table` did not make much difference relative to the base `dplyr` variant. 
...
#| include: false 
library(data.table)
game_users <- as.data.table(game_users)
game_actions <- as.data.table(game_actions)
game_purchases <- as.data.table(game_purchases)
exp_assignment <- as.data.table(exp_assignment)
```
